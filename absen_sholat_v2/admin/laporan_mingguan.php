...
$nama_bulan_num = [ 1=>'Januari',2=>'Februari',3=>'Maret',4=>'April',5=>'Mei',6=>'Juni',7=>'Juli',8=>'Agustus',9=>'September',10=>'Oktober',11=>'November',12=>'Desember' ];
...
if ($id_kelas_pilih != '') {
    ...
    if (!empty($d_info['nama_wali'])) { $wali_kelas_aktif = $d_info['nama_wali']; }
}

function ambil_daftar_kelas($koneksi) {
    if (isset($_SESSION['role']) && $_SESSION['role'] == 'wali_kelas') {
        $id_kw = $_SESSION['id_kelas_wali'];
        return mysqli_query($koneksi, "SELECT * FROM kelas WHERE id_kelas='$id_kw'");
    } else {
        return mysqli_query($koneksi, "SELECT * FROM kelas WHERE tahun_lulus IS NULL ORDER BY nama_kelas ASC");
    }
}
...
foreach ($daterange as $date) {
    if ($date->format("l") != 'Sunday') { $arr_tgl_header[] = $date; }
}
...
<html lang="id">
<head>
    <title>Laporan Mingguan - <?php echo $sekolah_nama; ?></title>
    ...
    <style>
        ...
        .tab-laporan { border-radius: 8px 8px 0 0; font-weight: bold; }
        @media print {
            ...
            .table-bordered, .table-bordered td, .table-bordered th { border: 1px solid black !important; padding: 2px 4px !important; }
            .table-dark, .table-dark th { background-color: #f8f9fa !important; color: black !important; }
            .bg-kuning { background-color: #f0f0f0 !important; color: black !important; }
            .header-print { text-align: center; border-bottom: 2px solid black; margin-bottom: 15px; padding-bottom: 10px; display: flex; align-items: center; justify-content: center; gap: 20px; }
            .logo-print { width: 60px; height: 60px; object-fit: contain; }
            .ttd-area { break-inside: avoid; page-break-inside: avoid; margin-top: 20px; }
...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-2 sidebar p-0 no-print">
            ...
            <a href="laporan_mingguan.php" class="menu-item active">LAPORAN</a>
        ...
        <div class="col-md-10 p-4">
            <div class="d-flex justify-content-between align-items-center mb-3 no-print">
                ...
                <div class="btn-group shadow-sm">
                    <a href="laporan_mingguan.php?kelas=<?php echo $id_kelas_pilih; ?>" class="btn tab-laporan btn-primary">Mingguan</a>
                    <a href="laporan.php?kelas=<?php echo $id_kelas_pilih; ?>" class="btn tab-laporan btn-outline-primary">Bulanan</a>
                    <a href="laporan_tahunan.php?kelas=<?php echo $id_kelas_pilih; ?>" class="btn tab-laporan btn-outline-primary">Tahunan</a>
                </div>
            </div>

            <div class="card mb-4 no-print shadow-sm">
                <div class="card-body">
                    <h5 class="fw-bold mb-3">Filter Laporan Mingguan</h5>
                    <form method="GET" class="row g-3">
                        <div class="col-md-4">
                            ...
                            <select name="kelas" class="form-select" <?php if (isset($_SESSION['role']) && $_SESSION['role'] == 'wali_kelas') echo 'style="pointer-events: none; background-color: #e9ecef;"'; ?>>
                                <?php
                                $q_kelas = ambil_daftar_kelas($koneksi);
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-3">
                        <div class="col-9">
                            <table class="table table-borderless table-sm m-0">
                                <tr><td width="100" class="fw-bold p-0">Kelas</td><td width="10" class="p-0">:</td><td class="text-nowrap p-0"><?php echo $nama_kelas_aktif; ?></td></tr>
                                <tr><td class="fw-bold p-0">Jurusan</td><td class="p-0">:</td><td class="text-nowrap p-0"><?php echo $jurusan_aktif; ?></td></tr>
                                <tr><td class="fw-bold p-0">Wali Kelas</td><td class="p-0">:</td><td class="text-nowrap p-0"><?php echo $wali_kelas_aktif; ?></td></tr>
                            </table>
                        </div>
                            ...
                            <thead class="table-dark">
                                ...
                                <tr>
                                    ...
                                    foreach ($arr_tgl_header as $dt) {
                                        ...
                                        $bg_head = ($hari_inggris == 'Sunday') ? "style='background-color:#dc3545; color:white;'" : "";
                                        echo "<th $bg_head class='py-1'><span style='font-size:11px; font-weight:bold; display:block;'>$hari_indo</span><span style='font-size:9px;'>($label_tgl)</span></th>";
                                    }
                                    ?>
                                    <th class="bg-success text-white" style="width:25px;">Hadir</th>
                                    <th class="bg-warning text-dark" style="width:25px;">Izin</th>
                                    <th class="bg-danger text-white" style="width:25px;">Alpa</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                if ($id_kelas_pilih != '') {
                                    ...
                                    while ($d = mysqli_fetch_array($q_siswa)) {
                                        $nisn = $d['nisn'];
                                        $hadir_count = 0; $izin_count = 0; $alpha_count = 0;
                    ...
                    <div class="row d-none d-print-flex ttd-area">
                        ...
                        <div class="col-4 text-start">
                            <p class="mb-1">Sungai Loban, <?php echo date('d') . ' ' . $nama_bulan_num[(int)date('n')] . ' ' . date('Y'); ?></p>
